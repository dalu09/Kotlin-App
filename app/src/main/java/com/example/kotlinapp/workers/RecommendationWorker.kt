...
class RecommendationWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result {
        ...
        val recommendedEvents = eventRepository.getUpcomingEventsBySport(mostViewedSport).getOrNull()

        val eventToShow = recommendedEvents?.firstOrNull() ?: return Result.success()
    ...
    private fun sendNotification(title: String, description: String) {
        ...
        val notification = NotificationCompat.Builder(applicationContext, "recommendation_channel")
            .setContentTitle("Evento recomendado: $title")
            .setContentText("A otros usuarios como tú les gustó este evento.")
